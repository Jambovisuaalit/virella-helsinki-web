...
import { createAdminSession, isAdminTokenConfigured, setAdminSession } from "@/lib/admin-auth";
...
export async function POST(request: Request) {
  ...
  if (!isAdminTokenConfigured() || !expected || supplied !== expected) {
  ...
  if (!session) {
    return NextResponse.redirect(new URL("/admin?error=1", origin), 303);
  }
